/**
 * @file ow_hal_f0.h
 * @brief STM32F0 backend: TIM1 (advanced) + DMA1 (channels 3/5) + PA8
 *
 * Header-only static inline implementation of the ow_hal_* interface for the
 * STM32F031. The 1-Wire bus runs on PA8 (TIM1_CH1 PWM output in open-drain
 * alternate function AF2); CH2 captures in indirect mode on the same pin and
 * DMA1 channel 3 moves CCR2 captures to memory, while channel 5 feeds CCR1
 * from a precomputed pulse buffer on the CC3 compare event (DMA1 channel 5,
 * the only TIM1 channel with a DMA request that can reload CCR1 on this
 * family). TIM1 runs in one-pulse mode (OPM) with the repetition counter
 * (RCR) batching N slots into a single update event (UIF).
 *
 * The CCR1 feed uses channel 3 (CC3DE) so the feed logic is identical across
 * the STM32F1 (DMA1 channel 6) and STM32F0 (DMA1 channel 5) backends.
 */
...
#else
#define OW_HAL_TIM_PRESCALER 47u /* 48MHz / 48 = 1MHz -> 1µs/tick */
#endif
 ...
 *       the '1' window. Mirrors ow_hal_f1.h. */
#if defined(HSI_8MHZ)
#define OW_HAL_IC2F_ARGS IC2F_1 /* fCK_INT, N=4 -> ~0.5µs @8MHz */
#else
#define OW_HAL_IC2F_ARGS IC2F_0, IC2F_1, IC2F_2 /* fDTS/4, N=8 -> ~0.67µs @48MHz */
#endif
...
__STATIC_FORCEINLINE void ow_hal_init(void) {
    ...
    /* PA8: alternate function mode, open-drain, AF2 (TIM1_CH1) */
    PA.MODER = (PA.MODER & ~GPIO_MODER_MODER8) | GPIO_MODER_MODER8_1;
    PA.OTYPER |= GPIO_OTYPER_OT_8;
    PA.AFR[1] = (PA.AFR[1] & ~GPIO_AFRH_AFSEL8) | (2u << GPIO_AFRH_AFSEL8_Pos);
}
...
__STATIC_FORCEINLINE void ow_hal_feed(const uint8_t* cmd, uint16_t slots) {
    ...
    ow_hal_update_event();
    D15.CCR = 0;
    D15.CPAR = (uint32_t)&T1.CCR1;
    D15.CMAR = (uint32_t)&cmd[1];
    D15.CNDTR = slots; /* Feed slots 2..N, then the trailing 0 (bus release) */
    D15.CCR = DMA_CCR(DIR, MINC, PSIZE_0, EN);
    T1.CR1 = TIM_CR1(OPM, CEN);
}
